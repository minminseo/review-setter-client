...
export const EditReviewDateModal = ({ isOpen, onClose, data }: EditReviewDateModalProps) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className=" border-b pb-2">復習日変更モーダル</DialogTitle>
                    <DialogDescription>
                        「{data.item.name}」の復習日を巻き戻します。
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                    {/* form以下のJSXは変更なし */}
                            ...
                            <div className="space-y-4 py-4">
                                ...
                                <FormField name="request_scheduled_date" control={form.control} render={({ field }) => (
                                    <FormItem className="flex flex-col">
                                        <FormLabel className="inline-block pointer-events-none select-none">新しい復習日</FormLabel>
                                        <Popover>
                                ...
                                <FormField name="is_mark_overdue_as_completed" control={form.control} render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="inline-block pointer-events-none select-none">後続の復習日の扱い</FormLabel>
                                        ...
                                        <FormDescription className="text-xs pt-2">
                                            ...
                                            変更後の日付によっては、算出された後続の復習日が今日より前になる場合があります。その場合、その復習日を完了状態にするか、今日に設定するかを選択してください。
                                        </FormDescription>
                                        <FormMessage />
                                    </FormItem>
                                )} />
